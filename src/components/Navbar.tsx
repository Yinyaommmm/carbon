...
const dropdownContentClasses = 'bg-white rounded-md p-1 shadow-lg z-50 min-w-[160px] border border-gray-100';
...
const DropdownLink: React.FC<DropdownLinkProps> = ({ path, name, dropdownItemClass }) => {
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md backdrop-blur-md bg-opacity-95">
      {/* 外部 div 铺满宽度 */}
      <div className="w-full px-4 sm:px-6 lg:px-8"> 
        ...
        <div className="flex justify-between items-center h-16 max-w-[1440px] "> 
          ...
          {/* 左侧：品牌 LOGO/名称 */}
          <NavLink to="/" className="block shrink-0 max-w-42 text-xl font-bold  text-gray-800 hover:text-green-600 transition-colors">
            <img src="/logo.jpg" alt="ABC ESG Link Logo" className=' object-contain'/>
          </NavLink>
          ...
          <div className="hidden md:flex space-x-1">
            ...
            <DropdownMenu.Root>
                ...
                <DropdownMenu.Trigger asChild>
                    ...
                    >
                        {t(dropdownItems.name)} <ChevronDown className="ml-1 w-3 h-3 transition-transform " />
                    </button>
                </DropdownMenu.Trigger>

              <DropdownMenu.Portal>
                <DropdownMenu.Content className={dropdownContentClasses} sideOffset={5} onCloseAutoFocus={(e) => e.preventDefault()}>
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              ...
              <span className="ml-1 text-sm font-medium">{t("cur_lang")}: {t('language_switch')}</span>
            </button>
